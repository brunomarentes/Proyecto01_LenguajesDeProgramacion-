...
-- z :: ASAValues
-- z = ClosureV "f" 
--   (AppV (FunV "x" (AppV (IdV "f") (FunV "y" (AppV (AppV (IdV "x") (IdV "x")) (IdV "y"))))) 
--         (FunV "x" (AppV (IdV "f") (FunV "y" (AppV (AppV (IdV "x") (IdV "x")) (IdV "y"))))))
--   []
...
helpMessage = unlines
  ...
  , "  (list lst)          " ++ colorVerde "- Lista literal"
...
testFunctions =
  ...
  -- TESTS PROGRESIVOS DE MAP
  , ("" ++ colorCyan "Map Test 4a: Doble de lista pequeña"
    , "(letrec (map (lambda (f lst) (if (null? lst) empty (pair (f (head lst)) (map f (tail lst)))))) (map (lambda (x) (* x 2)) (list 1 2)))"
    , "[2, 4]")

  , ("" ++ colorCyan "Map Test 4b: Doble de 3 elementos"
    , "(letrec (map (lambda (f lst) (if (null? lst) empty (pair (f (head lst)) (map f (tail lst)))))) (map (lambda (x) (* x 2)) (list 1 2 3)))"
    , "[2, 4, 6]")

  , ("" ++ colorCyan "Map Test 4c: Map con suma"
    , "(letrec (map (lambda (f lst) (if (null? lst) empty (pair (f (head lst)) (map f (tail lst)))))) (map (lambda (x) (+ x 10)) (list 1 2 3)))"
    , "[11, 12, 13]")

  , ("" ++ colorCyan "Map para listas (original)"
    , "(letrec (map (lambda (f lst) (if (null? lst) empty (pair (f (head lst)) (map f (tail lst)))))) (map (lambda (x) (* x 2)) (list 1 2 3 4 5)))"
    , "[2, 4, 6, 8, 10]")

  , ("" ++ colorCyan "Filter para listas"
    , "(letrec (filter (lambda (pred lst) (if (null? lst) empty (if (pred (head lst)) (pair (head lst) (filter pred (tail lst))) (filter pred (tail lst)))))) (filter (lambda (x) (> x 3)) (list 1 2 3 4 5 6)))"
    , "[4, 5, 6]")
  ]
...
-- MiniLisp.hs
evalSafe :: String -> IO (Either String ASAValues)
evalSafe input =
  catch (do
          let asa = desugar (parse (scanner input))
              asaVal = desugarV asa
